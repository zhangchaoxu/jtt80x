...
import co.xquick.jtt.entity.LoginRequest;
import co.xquick.jtt.entity.Message;
import co.xquick.jtt.entity.Vehicle;
import co.xquick.jtt.netty.NettyAccountConfig;
import co.xquick.jtt.utils.ByteBufPool;
import co.xquick.jtt.utils.DecimalConversion;
...
public class Jtt809Handler extends ChannelInboundHandlerAdapter {
    ...
    public Jtt809Handler(Jtt809NettyServer server, NettyAccountConfig nettyAccountConfig) {
        this.server = server;
        this.nettyAccountConfig = nettyAccountConfig;
        isMajor = true;
    }
    ...
    private void handlerBiz(ChannelHandlerContext ctx, Message msg) {
        ...
        if (op.isPresent()) {
            int locationNum = op.get().handlerBiz(nettyAccountConfig.getOrgKey());
    ...
    private void handlerLogin(ChannelHandlerContext ctx, Message msg) {
        ...
        // 转换登陆请求报文
        LoginRequest loginRequest = Jtt809Util.convertLoginRequest(msg);
        // 校验参数，获取响应码
        int responseCode = validateLogin(msg, loginRequest);
        LOGGER.info("接入码:{},用户:{} 登陆,登陆时间:{},密码:{},响应码:{}",
                msg.getMsgGesscenterId(), loginRequest.getUserId(), new Date(), loginRequest.getPassword(), responseCode);
        ...
        if (responseCode != JT809Constants.UP_CONNECT_RSP_SUCCESS) {
            ...
            LOGGER.info("接入码:{},用户:{} 登陆成功,登陆时间：{}", msg.getMsgGesscenterId(), loginRequest.getUserId(), new Date());
            server.setLoginStatus(LoginStatusEnum.runnable);
        }
        // 构建从链路
        buildClient(msg.getMsgGesscenterId(), loginRequest.getDownLinkIp(), loginRequest.getDownLinkPort(), verifyCode);
    }
    ...
    private int validateLogin(Message msg, LoginRequest request) {
        //判断接入码是否存在
        if (nettyAccountConfig.getMsgGesscenterId() == null) {
            return JT809Constants.UP_CONNECT_RSP_ERROR_02;
        }
        //判断用户是否存在
        if (nettyAccountConfig.getUserId() != request.getUserId()) {
            return JT809Constants.UP_CONNECT_RSP_ERROR_03;
        }
        //判断密码是否正确
        if (!nettyAccountConfig.getPassword().equals(request.getPassword())) {
